...
function Addq() {
    ...
    const rteChange = (content, delta, source, editor) => {
        final.options.map((o, pos) => pos === positions ? o.option = editor.getText() : o.option);
    };
    ...
    const richtextopt = (i, e) => {
        ...
        console.log(opts);
        if (opts[0].richTextEditor === false)
            opts[0].richTextEditor = true;
        else
            opts[0].richTextEditor = false;
        setOptarr(final.options.map((o) => o.richTextEditor));
    }
    ...
    const checkrad = (i) => {
        ...
        if (c === "MULTIPLE CHOICE")
            final.options.map((o, pos) => pos === i ? o.isCorrect = true : o.isCorrect = false);
        else {
            if (opts[0].isCorrect === false)
                opts[0].isCorrect = true;
            else
                opts[0].isCorrect = false;
        }
    }

    const success = () => toast("Question Added Succesfully....");
    const fail = () => toast("Question Not Added Failed....");
    const addopt = () => {
        setOpt([...opt, 1]);
        final.options = [...final.options, { "option": "", "isCorrect": false, "richTextEditor": false },];
        console.log(final);
    };
    const removeopt = (i) => {
            ...
            <Form noValidate validated={validated} onSubmit={handleSubmit}>
                ...
                <div className="container border">
                    <div className="row border-bottom">
                        <div className="col-sm "><h1 className="fw-bolder"><br />Add Question</h1></div>
                    </div><br />
                    <Toast>
                    <Toast.Header>
                        ...
                        <div className="col-sm">
                            ...
                            <InputGroup hasValidation>
                                <ReactQuill
                                    ...
                                    id="quill" className="form-control"
                            ...
                            <br /><br />
                            <button className="btn btn-link" onClick={callforquetext}> Rich Text Editor</button>
                        </div>
                    </div>
                    <br />
                    Options<br /><br />
                        ...
                        <div key={i}>
                            ...
                            <div className="input-group">
...

                                {
                                    optarr[i] ?
                                        <InputGroup hasValidation>
                                            <ReactQuill theme="snow" formats={formats} onFocus={() => setposition(i)} onChange={rteChange}
                                                className={"form-control"}
                                                placeholder="Enter Option..." modules={module = { "toolbar": optarr[i] && toolbar }} required />
                                            <Form.Control.Feedback type="invalid">
                                                Option is Required.
                                            </Form.Control.Feedback>
                                        </InputGroup>
                                        : <Form.Group md="4" >
                                            <InputGroup hasValidation>
                                                <Form.Control
                                                    ...
                                                    id="texts"
                                                    onChange={(e) => final.options.map((o, pos) => pos === i ? o.option = e.target.value : console.log(final))}
                                                    required
                                                />
                                                <Form.Control.Feedback type="invalid">
                                                    Option is Required.
                            ...
                            <Form.Group md="4">
                                ...
                                {" | "}
                                <button className="btn btn-link" onClick={() => richtextopt(i)}> Rich Text Editor</button>
                            </Form.Group>
                        </div>
                    ))}
                    ...
                    <div className="row">
                       ...
                       <ToastContainer/>
                        ...
                        <div className="col-2">
                            <button className="btn btn-outline-danger" onClick={cancelreq}>Cancel</button>
                        </div>
                    </div>
                </div>
            </Form>
